...
function TableAccordion_StudentProfile({ rows, accordionBreakpoint }: ITableAccordion_StudentProfile) {
        ...
        >
          ...
          <AccordionDetails>
            <Typography>
              ...
              {addOrdinalSuffix(row.grade!.valueOf())} | School:{" "}
              {row.activeSchoolCampuses!.length > 0 ? row.activeSchoolCampuses![0].name : "campus"} | Plan
              of Care: {row.activePlanOfCare?.type ? row.activePlanOfCare.type : "IEP"} | Plan of Care
              Duration: {dayjs(row.activePlanOfCare?.startDate).format(DAYJS_FORMATTER_DONTUSE)} -{" "}
              {dayjs(row.activePlanOfCare?.endDate).format(DAYJS_FORMATTER_DONTUSE)} | Progress: {50}
            </Typography>
          </AccordionDetails>
        </Accordion>
      ))}
        ...
        <TableBody>
          {rows.map((row: CaseloadStudentDisplay) => (
            <TableRow key={row.id} onClick={() => handleRowClick(row.studentIdGivenByState!)}>
              ...
              <TableCell>{addOrdinalSuffix(row.grade!.valueOf())}</TableCell>
              <TableCell>
                {row.activeSchoolCampuses!.length > 0 ? row.activeSchoolCampuses![0].name : "school"}
              </TableCell>
              <TableCell>{row.activePlanOfCare?.type ? row.activePlanOfCare.type : "IEP"}</TableCell>
