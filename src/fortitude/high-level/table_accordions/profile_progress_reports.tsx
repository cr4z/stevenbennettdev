...
function TableAccordion_ProgressReports({ rows, accordionBreakpoint }: ITableAccordion_ProgressReports) {
  ...
  return isAccordionView ? (
    ...
    <TableContainer>
      <Table>
        ...
        <TableBody>
          {rows.map((row: IDirty) => (
            <TableRow key={row.ProgressReportDate} onClick={() => handleRowClick(row.ProgressReportDate)}>
